...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Load image
        image, shape, org_img, filename = self.load_image(index)
        h, w = image.shape[:2]
        ...
        return torch.from_numpy(sample), target, shapes, org_img, filename

    def __len__(self):
        return len(self.filenames)

    def load_image(self, i):
        org_img = cv2.imread(self.filenames[i])
        h, w = org_img.shape[:2]
        r = self.input_size / max(h, w)
        if r != 1:
            image = cv2.resize(org_img,
                               dsize=(int(w * r), int(h * r)),
                               interpolation=resample() if self.augment else cv2.INTER_LINEAR)
            
        return image, (h, w), org_img, self.filenames[i]

    @staticmethod
    def collate_fn(batch):
        samples, targets, shapes, org_img, filename = zip(*batch)

        for i, item in enumerate(targets):
            item[:, 0] = i  # add target image index

        return torch.stack(samples, 0), torch.cat(targets, 0), shapes, org_img, filename
    ...
    def load_label(filenames):
        ...
        for filename in filenames:
            try:
                ...
                b = f'{os.sep}yolo_annotations{os.sep}'
