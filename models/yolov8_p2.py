...
from tools.utils import make_anchors
...
class DarkFPN(torch.nn.Module):
    def __init__(self, width, depth):
        ...
        self.csp1 = CSP(width[1]+width[2], width[1], depth[0], False)
        self.csp2 = CSP(width[1]+width[2], width[2], depth[0], False)

        self.conv1 = Conv(width[1], width[1], 3, 2)

    def forward(self, x):
        p1, p2 = x
        h1 = self.csp1(torch.cat([self.up(p2), p1], 1))
        h2 = self.csp2(torch.cat([self.conv1(h1), p2], 1))
        return h1, h2
...
class YOLO(torch.nn.Module):
    def __init__(self, width, depth, num_classes):
        ...
        self.head = Head(num_classes, (width[1], width[2]))
...
def yolo_v8_p2(num_classes: int = 80):
    depth = [1, 2, 2]
    width = [3, 16, 32, 64, 128, 256]
    return YOLO(width, depth, num_classes)
